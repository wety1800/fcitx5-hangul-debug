...
class TextKeyboard(
    ...
    private val hangulKeyMap = mapOf(
    ...
    override fun onInputMethodUpdate(ime: InputMethodEntry) {
        ...
        val wasHangulMode = isHangulMode
        isHangulMode = ime.languageCode == "ko"
        if (wasHangulMode != isHangulMode) {
    ...
    private fun transformPopupPreview(c: String): String {
        ...
        if (c[0].isLetter()) {
            return if (isHangulMode) {
                hangulKeyMap[c.uppercase()] ?: c
            } else {
                transformAlphabet(c)
            }
        }
    ...
    private fun updateAlphabetKeys() {
        textKeys.forEach {
            ...
            it.mainText.text = it.def.displayText.let { str ->
                ...
                if (isHangulMode) {
                    hangulKeyMap[str.uppercase()] ?: str
                } else {
